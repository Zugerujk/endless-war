...
help_responses = {
    ...
    "apartments": "Once you've gotten yourself an apartment, there are a variety of things you can do inside it. To enter your apartment, do **'!retire'** in the district your apartment is located in. Alternatively, you can **'!goto apt'**. To change the name and description of your apartment, do **'!aptname [name]'** and **'!aptdesc [description]'**, respectively. To place and remove furniture (purchasable in The Bazaar and elsewhere), do **'!decorate [furniture]'** and **'!undecorate [furniture]'**, respectively. You can store and remove items with **'!stow'** and **'!snag'**, respectively. To store and remove items from the fridge, do **'!fridge [item]'** and **'!unfridge [item]'**. To store and remove your slimeoid, do **'!freeze'** and **'!unfreeze'**, respectively. To store and remove items such as weapons and cosmetics, do **'!propstand [item]'** and **'!unstand [item]'**, respectively. To put away zines, do **!shelve [item]** and **!unshelve [item]**. To place crops into flower pots, do **pot [item]** and **unpot [item]** If you have a collection, you can **'!collect <collection> <item>'** to store certain items in that collection. You can place a collection with **'!decorate <collection>'**. To enter someone else's apartment, you can do **'!knock [player]'**, which will prompt them to let you in.",
    ...
    "collections": "**Collections** are items that can have items stored inside of them. You can obtain collections at The Museum. General collections can store **10** of any item. More specialized collections, which store **50** of **specific items**, with unique flavor text for those, can also be purchased. Placing a collection in your apartment will give it its own named line in your !look text.\n\nOnce you have a collection, you can **'!collect <collection> <item>'** to store an item in the collection. To remove the item from the collection, go to the Bazaar, and for 100,000 slime, you can **'!extract <collection> <item>'**. You can rename collections in your inventory with **'!renamecollection <collection> [name]'**. Finally, you can check the contents of a collection with **'!contents <collection>'**.",
    # Misc.
